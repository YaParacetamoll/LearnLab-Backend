...
try {
    switch ($_SERVER["REQUEST_METHOD"]) {
        case "GET":
            ...
            $courses = $db
                ...
                ->paginate(
                    "courses",
                    $page,
                    "c_id, c_name, c_hashed_password, c_description, c_banner_mime_type, c_updated_at"
                );
            $output["page"] = $page;
        ...
        case "PUT":
            ...
            $course = $db->getOne(
                "courses c",
                "c.c_id, c_name, c_description, c_code, c_hashed_password, c_updated_at, c_created_at, u_role, c_privacy, c_banner_mime_type"
            );
            $course["c_hashed_password"] = !is_null(
                $course["c_hashed_password"]
            );
        ...
        case "POST":
            ...
            if (
                ...
                $hash_password = key_exists("c_password", $_POST)
                    ? password_hash($_POST["c_password"], PASSWORD_DEFAULT)
                    : null;
                $data = [
                    "c_name" => $_POST["c_name"],
                    "c_code" => $c_code,
        ...
        case "DELETE":
            ...
            if (
                ...
                if ($role && $role == "INSTRUCTOR") {
                    ...
                    $clear_enrollment = $db->delete("enrollments")
                        ? true
                        : false;
                    $db->where("c_id", $_DELETE["c_id"]);
                    echo $clear_enrollment && $db->delete("courses")
                        ? jsonResponse(message: "successfully deleted")
                        : jsonResponse(400, "fail to delete course");
                } else {
