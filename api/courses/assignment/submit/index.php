    ...
    switch ($_SERVER["REQUEST_METHOD"]) {
        case "GET":
            ...
            if (key_exists("u_id", $_GET)) {
                ...
                $submissions = $db->getOne(
                    "submissions_assignment sa",
                    "a.a_due_date, a.a_score, a.a_name ,sa.* ,u_firstname, u_lastname, u_avatar_mime_type"
                );
                ...
                if (count($submissions["s_content"]->files) > 0) {
                    ...
                    $submissions["s_content"]->files = $db->get(
                        "files",
                        null,
                        $cols
                    );
                ...
                $col = ["a_id", "u.u_firstname", "u.u_lastname", "s_datetime"];
                $submissions = $db->rawQuery(
                    "SELECT e.u_id, s.a_id, u.u_firstname, u.u_lastname, u.u_avatar_mime_type, s.score, s_datetime FROM enrollments e LEFT JOIN
        ...
        case "PUT": //submit
            ...
            foreach (array_keys($_PUT) as $key) {
                $data[$key] =
                    $key == "s_content"
                        ? json_encode($_PUT[$key])
                        : $_PUT[$key];
            }
            echo $db->insert("submissions_assignment", $data)
                ? jsonResponse(message: "บันทีกการส่งเรียบร้อย")
                : jsonResponse(400, "ไม่สามารถบันทีกการส่งได้");
            break;
        case "PATCH": //ตรวจงาน
            $_PATCH = json_decode(file_get_contents("php://input"), true);
            if (
                !isset($_PATCH) &&
                !key_exists("u_id", $_PATCH) &&
                key_exists("a_id", $_PATCH)
            ) {
                echo jsonResponse(400, "ค่าที่ให้มาไม่ครบหรือไม่ถูกต้อง");
                die();
            }
            ...
            echo $db->update("submissions_assignment", $data)
                ? jsonResponse(message: "บันทึกการตรวจเรียบร้อย")
                : jsonResponse(400, "ไม่สามารถบันทีกได้");
            break;
