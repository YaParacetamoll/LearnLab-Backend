    ...
    switch ($_SERVER["REQUEST_METHOD"]) {
        case "GET":
            if (
                ...
                if ($result) {
                    ...
                    $db->join(
                        "enrollments e",
                        "e.u_id=p.u_id AND e.c_id=p.c_id",
                        "LEFT"
                    );
                    $cols = [
                        "p_id, p_created_at, p_updated_at, p_title, p_content, p_item_list, p_type, p.u_id, e.u_role, u_firstname, u_lastname, u_avatar_mime_type",
                    ];
                    $db->orderBy("p.p_created_at", "desc");
                    $posts = $db->get("posts p", null, $cols);
                    foreach (array_values($posts) as $i => $obj) {
                        $posts[$i]["u_avatar"] = !is_null(
                            $posts[$i]["u_avatar_mime_type"]
                        );
                        $posts[$i]["p_item_list"] = json_decode(
                            $posts[$i]["p_item_list"]
                        );
                        ...
                        if (count($posts[$i]["p_item_list"]->assignments) > 0) {
                            $db->where(
                                "a_id",
                                $posts[$i]["p_item_list"]->assignments,
                                "IN"
                            );
                            $cols = ["a_id", "a_name", "a_due_date", "a_score"];
                            $posts[$i]["p_item_list"]->assignments = $db->get(
                                "assignments",
                                null,
                                $cols
                            );
                        ...
                        if (count($posts[$i]["p_item_list"]->files) > 0) {
                            $db->where(
                                "f_id",
                                $posts[$i]["p_item_list"]->files,
                                "IN"
                            );
                            $db->orderBy("f_name", "asc");
                            $cols = ["f_id", "f_name", "f_mime_type"];
                            $posts[$i]["p_item_list"]->files = $db->get(
                                "files",
                                null,
                                $cols
                            );
                        ...
                        if (count($posts[$i]["p_item_list"]->quizzes) > 0) {
                            $db->where(
                                "q_id",
                                $posts[$i]["p_item_list"]->quizzes,
                                "IN"
                            );
                            $cols = ["q_id", "q_name", "q_due_date"];
                            $posts[$i]["p_item_list"]->quizzes = $db->get(
                                "quizzes",
                                null,
                                $cols
                            );
                    ...
                    echo jsonResponse(
                        400,
                        "You are not enrolled on that course."
                    );
                }
            } else {
        ...
        case "PUT":
            ...
            if (
                ...
                if (
                    $result &&
                    in_array($result["u_role"], ["TA", "INSTRUCTOR"])
                ) {
                    $p_content = key_exists("p_content", $_PUT)
                        ? $_PUT["p_content"]
                        : null;
                    $ct_id = key_exists("ct_id", $_PUT) ? $_PUT["ct_id"] : null;
                    $p_item_list = key_exists("p_item_list", $_PUT)
                        ? json_encode($_PUT["p_item_list"])
                        : null;
                    $p_show_time = key_exists("p_show_time", $_PUT)
                        ? $_PUT["p_show_time"]
                        : null;
                    $data = [
                        "c_id" => $_PUT["c_id"],
                    ...
                    echo $db->insert("posts", $data)
                        ? jsonResponse(message: "Post created successfully")
                        : jsonResponse(400, "Failed to create post");
                } else {
            ...
            if (
                ...
                if (intval($JWT_SESSION_DATA["u_id"]) == $post_u_id) {
                    ...
                    foreach (array_keys($JSON_DATA) as $key) {
                        $data[$key] =
                            $key == "p_item_list"
                                ? json_encode($JSON_DATA[$key])
                                : $JSON_DATA[$key];
                    }
                    $db->where("p_id", $JSON_DATA["p_id"]);
                    echo $db->update("posts", $data)
                        ? jsonResponse(message: "Post edited successfully")
                        : jsonResponse(400, "Fail to edit post.");
                } else {
        ...
        case "DELETE":
            ...
            if (
                ...
                if (
                    ...
                    $p_item_list = json_decode(
                        $db->getValue("posts", "p_item_list")
                    );
                    ...
                    $db->where("p_id", $_DELETE["p_id"]);
                    echo $db->delete("posts")
                        ? jsonResponse(message: "ลบโพสต์เรียบร้อยแล้ว")
                        : jsonResponse(400, "ไม่สามารถลบโพสต์ได้");
                } else {
