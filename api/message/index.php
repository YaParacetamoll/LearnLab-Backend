...
try {
    switch ($_SERVER['REQUEST_METHOD']) {
        case "GET":
            ...
            if (isset($_GET) && key_exists("c_id", $_GET)) {
                ...
                if ($messages) {
                    ...
                    echo jsonResponse(message: "Post นี้ยังไม่มีการ Comment");
                }
            } else {
        ...
        case "DELETE": //delete?
            ...
            $db->where("m_id", $_DELETE["m_id"]);
            echo (!strcmp($m_sender, $_SESSION['u_id']) && $db->delete("messages")) ? jsonResponse(message: "ลบข้อความเรียบร้อย") : jsonResponse(400, "ไม่สามารถลบข้อความได้");
            break;
    }
} catch (Exception $e) {
